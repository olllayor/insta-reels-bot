...
export function generateSquirclePath(size: number, n: number = 5): string {
  const r = size / 2;
  const cx = r;
  const cy = r;
  const points = 512;
...
export function generateMaskSvg(size: number, background?: string): string {
  const path = generateSquirclePath(size, 5);
  const backgroundRect = background ? `\n      <rect width="${size}" height="${size}" fill="${background}"/>` : '';
  return `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision">${backgroundRect}
      <path d="${path}" fill="white" shape-rendering="geometricPrecision"/>
    </svg>`;
}
