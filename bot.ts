...
bot.on('message:video', async (ctx) => {
  ...
  try {
    ...
    // Reject oversized files before attempting the download.
    if (file.file_size && file.file_size > MAX_DOCUMENT_UPLOAD_BYTES) {
      ...
      await ctx.api.editMessageText(
        processingMsg.chat.id,
        processingMsg.message_id,
        `❌ Video is too large. Maximum supported size is ${MAX_DOCUMENT_UPLOAD_SIZE_MB}MB.`,
      );
      return;
    }
    ...
    const outputPath = join(downloadedFile.dir, `squircle-${Date.now()}.mp4`);
    ...
    // ENOENT means the ffmpeg/ffprobe binary was not found on the system.
    // Avoid the broader string-includes check — it would catch any runtime ffmpeg
    // error (filter failures, corrupt input, etc.) and show the wrong message.
